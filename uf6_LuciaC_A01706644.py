...
Original file is located at
    https://colab.research.google.com/drive/124HHuA9Oa1R0gXEfGYyeu4njYcaAmw0H
"""
...
st.subheader('Crime location in San Francisco')
st.map(subset_data)

st.subheader('Crimes occurred per day of the week')
st.bar_chart(subset_data['Day'].value_counts().sort_values(ascending=False))

st.subheader('Crimes occurred per date')
st.line_chart(subset_data['Date'].value_counts().sort_index())

st.subheader('Type of crimes committed')
st.bar_chart(subset_data['Incident Category'].value_counts().sort_values(ascending=False))

agree = st.button('Click to show Incident Subcategories')
if agree:
    st.subheader('Subtype of Crimes committed')
    st.bar_chart(subset_data['Incident Subcategory'].value_counts().sort_values(ascending=False))
...
fig = px.pie(resolution_counts, values='Count', names='Resolution', title='Resolution Status')
st.plotly_chart(fig)
